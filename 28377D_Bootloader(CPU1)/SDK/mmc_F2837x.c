...
/*-----------------------------------------------------------------------*/
char spi_start[6] = {0,0};
long spi_tm_cnt[6] = {0,0};
unsigned long spi_pre_t[6];
unsigned long spi_cur_t[6];
unsigned long spi_delta_t[6];

void Time_Cal(char i)
{
    if(spi_start[i] == 0)
    {
        spi_pre_t[i] = CpuTimer2Regs.TIM.all;
        spi_start[i] = 1;
    }

    else
    {
        spi_cur_t[i] = CpuTimer2Regs.TIM.all;

        if( spi_cur_t[i] > spi_pre_t[i])
        {
            spi_delta_t[i] = spi_pre_t[i] + (0xffffffff - spi_cur_t[i]);
        }
        else
        {
            spi_delta_t[i] = spi_pre_t[i] - spi_cur_t[i];
        }

        if(spi_delta_t[i] > 10000)
        {
            spi_tm_cnt[i] ++;
        }
...
void xmit_spi (BYTE dat)    //EDITED
{
    ...
    while(SpicRegs.SPISTS.bit.INT_FLAG !=1)    //May be possible to switch to '!SpicRegs.SPISTS.bit.INT_FLAG'
    {
        ...
        if(spi_tm_cnt[0] > 500)
        {
            SpicRegs.SPIDAT = spi_tm_cnt[0] = 0;
        }
    }
    spi_pre_t[0] = spi_cur_t[0];
...
BYTE rcvr_spi (void)        //EDITED
{
    ...
    while(SpicRegs.SPISTS.bit.INT_FLAG !=1)    //May be possible to switch to '!SpicRegs.SPISTS.bit.INT_FLAG'
    {
        ...
        if(spi_tm_cnt[1] > 500)
        {
            SpicRegs.SPIDAT = spi_tm_cnt[1] = 0;
        }

    }
    rcvdat = (SpicRegs.SPIRXBUF & 0xFF);        //Read Transferred data
    spi_pre_t[1] = spi_cur_t[1];
    return (BYTE)rcvdat;
}
...
void power_on (void)    //EDITED
{
    ...
    SpicRegs.SPIBRR.all = 0;                       //update value to proper setting for correct bitrate ( current: ~500kHz)
...
{
    return (((unsigned long)r_time.Read_year - 1980) << 25) // Year
           | ((unsigned long)r_time.Read_mon << 21)         // Month
           | ((unsigned long)r_time.Read_day << 16)         // Day
           | ((unsigned)r_time.Read_hour << 11)             // Hour
           | ((unsigned)r_time.Read_min << 5)               // Min
           | ((unsigned)r_time.Read_sec >> 1)               // Sec
        ;
}
